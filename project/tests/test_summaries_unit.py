...
def test_create_summary(test_app, monkeypatch):
    test_request_payload = {"url": "http://example.com"}
    test_response_payload = {"id": 1, "url": "http://example.com"}
...
def test_read_all_summaries(test_app, monkeypatch):
    ...
    async def mock_gat_all():
        return test_data

    monkeypatch.setattr(crud, "get_all", mock_gat_all)
...
def test_remove_summary(test_app, monkeypatch):
    ...
    assert response.status_code == 200
    assert response.json() == {"id": 1, "url": "http://foo.bar"}
